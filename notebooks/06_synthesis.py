...
# Preferred headline: BJS (most robust to heterogeneity + cluster SE, and
# in this sample lines up with TWFE to the third decimal).
headline = did["bjs"]
...
reconciled = {
    ...
    "panel": {
        ...
        "treated_units": panel_summary["treated_units"],
        "n_treated": panel_summary["n_treated_units"],
        "treatment_date": panel_summary["treatment_date"],
    ...
    "headline_att": {
        ...
        "n": headline["n"],
        "interpretation": (
            "BJS DiD: treated CDs experienced a reduction of "
            f"{abs(headline['att']):.1f} rodent complaints per community "
            f"district per month post-2023-07-01 (95% CI "
...
jc.save(reconciled, "artifacts/reconciled_findings.json",
...
def _fmt_p(p_):
    if p_ is None: return "—"
    if p_ < 0.001: return "&lt; .001"
    return f"{p_:.3f}"
...
lines = [
    ...
    "## Headline",
    "",
    f"**BJS DiD ATT = {h['att']:+.2f}** Rodent complaints per community district per month "
    f"(*SE* = {h['se']:.2f}, 95% CI [{h['ci_95_low']:+.2f}, {h['ci_95_high']:+.2f}], "
    ...
    f"- **Treatment**: mandatory bin containerization pilot, effective "
    f"{p['treatment_date']}.",
    f"- **Treated units** ({p['n_treated']}): "
    + ", ".join(p["treated_units"]) + ".",
    "",
    ...
    "",
    "All four estimators agree in sign. TWFE and BJS are numerically "
    "equivalent here because adoption is a single cohort; CS and SA "
    ...
    f"{_fmt_p(rob['placebo_p_bjs'])} | Non-significant placebo "
    f"(rejection would argue anticipation or pre-existing trend). |",
    f"| Log-outcome TWFE | {rob['log_outcome_coef']:+.3f} "
    f"({rob['log_outcome_pct_change']:+.1f}%) | "
    f"{_fmt_p(rob['log_outcome_p'])} | "
    f"Multiplicative specification: direction consistent, magnitude smaller. |",
    f"| Post-COVID sample (2022-01 →) | {rob['post_covid_att_bjs']:+.2f} | "
    f"{_fmt_p(rob['post_covid_p_bjs'])} | Strengthens headline; COVID "
    f"lockdowns are not driving the effect. |",
    f"| Manhattan-only controls (BJS) | {rob['manhattan_only_att_bjs']:+.2f} | "
    f"{_fmt_p(rob['manhattan_only_p_bjs'])} | Same sign; wide CIs due to "
    f"small control set (6 CDs). |",
    "",
    "## Diagnostics",
    "",
    ...
    f"{'**Reject** flat pre-trends (see §5.3).' if dg['pre_trends_F_reject'].startswith('reject') else 'Fail to reject.'} |",
    f"| Breusch-Pagan | *p* {_fmt_p(dg['bp_p'])} | "
    f"{'Heteroskedastic residuals.' if dg['bp_heteroskedastic'] == 'heteroskedastic' else 'Homoskedastic.'} |",
    f"| TWFE *R*² | {dg['r_squared']:.3f} | Within-panel variance largely absorbed by fixed effects. |",
    f"| Shapiro-Wilk | sampled *p* {_fmt_p(dg['bp_p'])} | Non-normal residuals — count-data feature. |",
    "",
    "## Balance (pre-treatment)",
    "",
    f"Pre-period (2020-01 → 2023-06) mean monthly complaints: "
    f"{bal['pre_treated_mean']:.1f} (treated) vs. {bal['pre_control_mean']:.1f} "
    f"(control). Welch *t* = {bal['welch_t']:.2f}, *p* {_fmt_p(bal['welch_p'])}, "
    f"Cohen's *d* = {bal['cohens_d']:.2f} (small-to-medium). "
    "Treated community districts carried higher pre-period complaint "
    "rates; the DiD identifying variation is the change from this "
    "elevated baseline.",
    "",
    "---",
...
out.write_text("\n".join(lines))
jc.save({"path": str(out), "n_lines": len(lines)},
...
lines = [
    ...
    "| :--- | :--- | :--- | :--- |",
    f"| 1 | **Parallel trends** (flat pre-period leads) | **Violated** | Joint *F* = {dg['pre_trends_F_stat']:.2f}, *p* {_fmt(dg['pre_trends_F_p'])}. Treated CDs climb faster pre-2023; see §5.3 limitations. |",
    f"| 2 | **No anticipation** (null placebo at t₀-12mo) | **Pass** | Placebo BJS ATT = {rob['placebo_att_bjs']:+.2f}, *p* {_fmt(rob['placebo_p_bjs'])}. |",
    f"| 3 | **Sign agreement across estimators** | **Pass** | All four (TWFE, CS, SA, BJS) agree on negative sign. |",
    ...
    f"| 7 | **COVID-sample restriction** | **Strengthens** | Post-2022 subsample BJS ATT = {rob['post_covid_att_bjs']:+.2f}, *p* {_fmt(rob['post_covid_p_bjs'])}. |",
    f"| 8 | **Alternative control (MN-only)** | **Consistent** | Sign agreement; wide CI due to *N* = 6 controls. |",
    ...
    "robust to several alternative specifications, but **the parallel-"
    "trends violation is a material limitation** — treated CDs were on "
    "a steeper trajectory pre-treatment. Readers should interpret the "
    "point estimate as an upper bound on the true ATT. A fuller "
    "identification strategy (synthetic control on lower-Manhattan CDs) "
...
out.parent.mkdir(parents=True, exist_ok=True)
out.write_text("\n".join(lines))
jc.save({"path": str(out), "n_rows": 10},
